...
SELECT
  ...
  CASE
    WHEN COALESCE(er.estimate_total, 0) + COALESCE(cr.approved_co_total, 0) = 0
      THEN 0
    ELSE (
      (COALESCE(er.estimate_total, 0) + COALESCE(cr.approved_co_total, 0) - COALESCE(l.labor_cost, 0))
      / (COALESCE(er.estimate_total, 0) + COALESCE(cr.approved_co_total, 0))
    )
  END                                         AS realized_margin_ratio,
  CASE
    WHEN COALESCE(er.estimate_total, 0) + COALESCE(cr.approved_co_total, 0) = 0
      THEN 0
    ELSE COALESCE(l.labor_cost, 0)
      / (COALESCE(er.estimate_total, 0) + COALESCE(cr.approved_co_total, 0))
