...
-- Replace function body only (same signature)
...
BEGIN
  ...
  -- Time Entry Inclusion Contract with org-level rate fallback
  -- Priority: project_members.cost_rate > organization_memberships.hourly_cost_rate > 0
  SELECT
    COALESCE(SUM(te.duration_hours), 0),
    COALESCE(SUM(te.duration_hours * COALESCE(pm.cost_rate, om.hourly_cost_rate, 0)), 0),
    COALESCE(SUM(CASE
      WHEN pm.user_id IS NOT NULL AND COALESCE(pm.cost_rate, om.hourly_cost_rate, 0) = 0
