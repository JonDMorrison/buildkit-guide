-- Fix rpc_get_system_integrity_issues: the time_entries columns are
-- check_in_at / check_out_at (not check_in / check_out). The original
-- function referenced the wrong names and was failing with
-- "column te.check_out does not exist" — visible in Postgres error logs.
-- Fix already applied to production via MCP; this file syncs the repo.
...
BEGIN
  ...
  -- 1. Unrated labor hours (check_out -> check_out_at, check_in -> check_in_at)
  SELECT
    COALESCE(SUM(EXTRACT(EPOCH FROM (te.check_out_at - te.check_in_at)) / 3600.0), 0),
    COUNT(*)
  ...
  -- 4. Pending receipts older than 7 days
