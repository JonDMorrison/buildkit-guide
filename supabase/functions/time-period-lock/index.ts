...
serve(async (req) => {
  ...
  try {
    ...
    // Call RPC
    const { data, error } = await adminClient.rpc('rpc_lock_timesheet_period', {
    ...
    if (error) {
      ...
      const message = error.message || 'Failed to lock period';
      let code = 'DB_ERROR';
      let status = 500;
      ...
      if (message.includes('not found')) {
        code = 'NOT_FOUND';
        status = 404;
      } else if (message.includes('Not in org') || message.includes('Only HR/Admin')) {
        code = 'FORBIDDEN';
        status = 403;
      } else if (message.includes('must be approved')) {
        code = 'INVALID_STATE';
        status = 400;
      }
    ...
    console.log('Period locked:', period_id);
    return new Response(
