...
| G-010 | Overlapping time entries detected | Admin | Worker A has 2 closed entries on same day with overlapping check_in/check_out ranges | 1. View Data Health | "Overlapping Time Entries" section shows ≥1 row with worker name, date, and overlap count | Overlap not detected; false positive on adjacent (non-overlapping) entries | `SELECT a.id, b.id FROM time_entries a JOIN time_entries b ON a.user_id=b.user_id AND a.id<b.id AND a.check_out_at>b.check_in_at AND a.check_in_at<b.check_out_at WHERE a.project_id IN (:pids) AND a.status='closed'` — count matches UI | P1 |
| G-011 | No false positive on adjacent entries | Admin | Worker has entry 08:00–12:00 and 12:00–17:00 (no overlap) | 1. View Data Health | "Overlapping Time Entries" shows 0 for this worker | False positive overlap detected | Adjacent entries have check_out_at = next check_in_at, which should NOT overlap (half-open interval) | P1 |
| G-012 | Cross-project task reference — invariant zero | Admin | All task-linked time entries reference tasks within same project | 1. View Data Health | "Cross-Project Task References" shows 0 | Any non-zero count is a P0 data integrity bug | `SELECT te.id FROM time_entries te JOIN tasks t ON t.id=te.task_id WHERE te.project_id != t.project_id` — must return 0 rows | P0 |
